...
There were four measured parameters: Anxious, Worry, Down and Interest. As per CDC, a person can be considered suffering from anxiety syndrome if the sum of the Anxious and Worry scores is greater than or equal to 4. Similarly, a person can be considered suffering from depression syndrome if the sum of the Down and Interest scores is greater than or equal to 4.

We wrangled the data to calculate the percentage of people suffering from anxiety and depression syndrome by state and week for this project.
...
To find out the reasons for these anomalies it is important to examine how the government heralded the public opinion. Below are the word clouds of Tweets from US senators for the weeks in question.
...
```{r covid-cases-graph, echo=FALSE, message=FALSE, fig.height = 4.5, fig.width = 6, fig.align = 'center'}
...
![](img/control-spatial.jpg){width=100%}

![](img/cases-anxiety-spatial.jpg){width=100%}
...
```{r mental-health-revisited, echo=FALSE, message=FALSE}
# import data
mental_health <- read_csv("blog-project/data/mental-health/mental_health.csv") %>%
  filter(State == "National")
...
Looking at the most common words in tweets regarding COVID, there are some obvious words that repeats and skews the data including words like: health, pandemic, coronavirus etc. After filtering out these words here is what we are left with.

![](img/wordclouds.png){width=100%}
![](img/wctotal.png){width=100%}

![](img/wctotal_bar.png){width=100%}
...
```{r word-trends, echo=FALSE, message=FALSE}
word_cloud <- read_csv2("blog-project/data/textual/senator_wordcloud.csv")
sentiments <- read_csv2("blog-project/data/textual/twitter_sentiments.csv")
tweets <- read_csv2("blog-project/data/textual/senator_tweets_covid.csv")
...
## Sentiments
```{r sentiment-total, echo=FALSE, message=FALSE}
...
```{r sentiment-trend, echo=FALSE, message=FALSE}
temp_sent <- sentiments %>%
...
```{r covid-cases-graph-revisited, echo=FALSE, message=FALSE, fig.height = 4.5, fig.width = 6, fig.align = 'center'}
timeline_graph <- read_csv("blog-project/shiny/timeline_graph.csv") %>%
  filter(response == "Covid Cases", state == "National")
...
There were many limitations that we came across while investigating our research questions due to either our limited skillset in statistical analysis and creating data visualization and issues with the data itself. Therefore, we strongly urge the audience to take our results with caution. 
