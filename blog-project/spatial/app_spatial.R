...
MergedStatesSentiment <- read_csv2("MergedStatesSentiment.csv")
...
server <- function(input, output){
  ...
  data_for_week_sentiment <- reactive({
    data <- filter(MergedStatesSentiment, week %in% input$weekslider,
  ...
  output$sentimentmap <- renderPlot({ 
    ggplot() +
      geom_polygon(data = data_for_week_sentiment(),
                   aes_string(x = "long", y = "lat", group = "group", fill = "percentage_of_national_sentiment"),
                   color = "black", size = 0.2) +
