...
data <- senator_tweets %>% 
      filter(grepl("COVID",text)) %>%
      filter(created_at >= as.Date(ymd(20090904)) & created_at <= as.Date(ymd(20110904))) %>%
      unnest_tokens(input = text, output = tokens) %>% 
      #get rid of stop words
      anti_join(stop_words, by = c("tokens" = "word")) %>%
      count(tokens) %>%
...
wordcloud(words = data$tokens,freq = data$n, max.words = 30)

library(usmap)

word_count <- read_csv2("data/textual/twitter_sentiments.csv")
...
plot_usmap(data = word_count, values = "percentage") +
  scale_fill_continuous(low = "white", high = "red", name = "Positive Sentiment", label = scales::comma)
