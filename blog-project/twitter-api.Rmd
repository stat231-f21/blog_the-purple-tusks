...
word_count <- senator_covid_words %>%
  ...
  mutate(sum = positive+negative+anger+anticipation+disgust+fear+joy+sadness+surprise+trust) %>%
  pivot_longer(-c(week,sum, state),names_to = "sentiments", values_to = "count")
...
word_count_national <- senator_covid_words %>%
  ...
  pivot_wider(names_from = sentiment, values_from = total, values_fill = 0) %>%
  mutate(sum = positive+negative+anger+anticipation+disgust+fear+joy+sadness+surprise+trust) %>%
  pivot_longer(-c(week,sum),names_to = "sentiments", values_to = "count")
